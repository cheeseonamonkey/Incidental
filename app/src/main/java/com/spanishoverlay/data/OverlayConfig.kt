...
data class OverlayConfig(
    ...
    fun persist(prefs: SharedPreferences) {
        prefs.edit().apply {
            ...
            putBoolean("normalization_enabled", normalizationEnabled)
            putFloat("repeat_recent_weight", repeatRecentWeight)
            putInt("overlay_ttl_ms", overlayTtlMs)
            putInt("fade_in_ms", fadeInMs)
            putInt("fade_out_ms", fadeOutMs)
            putInt("show_delay_ms", showDelayMs)
            putInt("debounce_ms", debounceMs)
            putBoolean("rescan_after_clear_events", rescanAfterClearEvents)
            ...
            putBoolean("allow_overlay_overlap", allowOverlayOverlap)
