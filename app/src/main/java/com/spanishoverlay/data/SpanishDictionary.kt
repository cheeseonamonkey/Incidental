...
object SpanishDictionary {
    ...
    @Volatile private var phraseIndex: List<Pair<String, DictionaryEntry>> = emptyList()
    ...
    fun ensureLoaded(context: Context) {
        ...
        synchronized(this) {
            ...
            for (i in 0 until arr.length()) {
                ...
                val pos = runCatching { PoS.valueOf(obj.optString("pos", PoS.OTHER.name)) }.getOrDefault(PoS.OTHER)
                val c = obj.optInt("c", 0)
                val altArr = obj.optJSONArray("alt")
                val alts = if (altArr == null) emptyList() else (0 until altArr.length()).mapNotNull { altArr.optString(it).takeIf { alt -> alt.isNotBlank() } }
