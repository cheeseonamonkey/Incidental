...
class PipelineTest {
    ...
    @Test fun `dictionary lookup finds base and inflected forms`() {
        assertNotNull(SpanishDictionary.findAny("house"))
        assertNotNull(SpanishDictionary.findAny("houses"))
        assertNotNull(SpanishDictionary.findAny("running"))
        assertNull(SpanishDictionary.findAny("xyznonexistent"))
    }

    @Test fun `dictionary is case insensitive`() {
        assertNotNull(SpanishDictionary.findAny("House"))
        assertNotNull(SpanishDictionary.findAny("WATER"))
    }
    ...
    @Test fun `complexity filter respects range`() {
        ...
        val entry3 = DictionaryEntry("ephemeral", "efimero", PoS.ADJECTIVE, 3)
    ...
    @Test fun `fraction mode is deterministic`() {
        // Same text -> same hash -> same selection
        val entry = SpanishDictionary.findAny("house")
        assertNotNull(entry)
    }

    @Test fun `dictionary has expected size`() {
        assertTrue(SpanishDictionary.size() >= 90)
    }
}
